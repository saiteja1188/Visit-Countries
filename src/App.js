...
class App extends Component {
  ...
  addVisitedCountry = id => {
    ...
    this.setState({countriesList: visitedCountry})
  }

  deleteVisitCountry = id => {
    const {countriesList} = this.state
    const filterList = countriesList.filter(eachDelete => eachDelete.id !== id)
    ...
    return (
      <div className="app-container">
        ...
        <ul className="names-list">
          {countriesList.map(eachName => (
            <CountriesList
              key={eachName.id}
              countriesNameDetails={eachName}
        ...
        {updatedList.length > 0 ? (
          ...
          <div className="no-country">
            <p className="main-heading">No Countries Visited Yet!</p>
          </div>
        )}
      </div>
    )
